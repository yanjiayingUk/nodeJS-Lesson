...
http.createServer((req, res) => {
    ...
    if (pathName == "/detail") {
        ...
        listAdd(req, res, pathName);
    }
    else if (pathName == "/listfont") {
        var listStr = JSON.stringify(chapterList);
        res.writeHead(200, { "Content-Type": "text/plain" });
        res.write(listStr);
        res.end();
...
function enterht(res, urlObj) {
    ...
    for (var i = 0; i < userList.length; i++) {
        jsonArr[i] = userList[i];
    }
    var user = urlObj.query.username;
    var password = urlObj.query.pwd;
    console.log(user);
    console.log(password);
    for (var i = 0; i < userList.length; i++) {
        jsonArr[i] = userList[i];
        if (jsonArr[i].username == user && jsonArr[i].pwd == password) {
            showList(res);
        }
    }
}

function listAdd(req, res) {
    var formData = "";
    ...
    req.on("end", function () {
        ...
        // var listStr=JSON.stringify(formObj);
        // console.log(formObj);
        var a = {};
        a.chapterId = chapterList.length + 1;
        a.chapterName = formObj.title;
        a.imgPath = "images/1442539025939884-lp.jpg";
        a.chapterDes = formObj.content;
        a.chapterContent = formObj.content;
        var birthTimer = new Date();
        a.publishTimer = birthTimer.getFullYear() + "-" + (birthTimer.getMonth() + 1) + "-" + birthTimer.getDate();
        a.author = "admin";
        a.views = 100;
        res.end();
        var c = chapterList.push(a);
